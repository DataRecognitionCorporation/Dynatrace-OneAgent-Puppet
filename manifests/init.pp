...
# OneAgent Host Configuration Parameters
  ...
  Optional[Enum['fullstack', 'infra-only', 'discovery']] $monitoring_mode  = $dynatraceoneagent::params::monitoring_mode,
  ...
  String $oneagent_networkzone_config_file                                 = $dynatraceoneagent::params::oneagent_networkzone_config_file,
    ...
    if $version == 'latest' {
      if ($::kernel == 'Linux') or ($::osfamily  == 'AIX') {
        ...
        $version_link = "-H 'Authorization: Api-Token ${paas_token}' ${tenant_url}${api_path}${os_type}/${installer_type}/latest/metainfo"
      } else {
